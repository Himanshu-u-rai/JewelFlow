...
| staging 2 | checkout (`config:clear`) | **release blocker**: `PaymentRaceHarness` under `--no-dev` | restored; `5271eb1`. 6 errors 23:44:48–23:45:13Z: the staging worker failing to boot at each restart (inferred from their 5-second spacing and 7 starts in its journal; a fatal error carries no trace); no 5xx in the access log |
...
| production 1 | checkout (`package:discover`) | production's `.env` unreadable by www-data; `config:clear` as www-data removed the only readable config; the boot guard threw | **02:32:57–02:35:15Z in maintenance (2 m 18 s)**, and the ops-alerts worker down: the 30 errors logged 02:33:01–02:35:13Z are that worker failing to boot at each 5-second systemd restart (every trace enters through `artisan`). *Corrected:* this row first said visitors got application errors instead of the maintenance page; the access log shows no web request in the window except my two checks after the restore (02:35:15Z, both 200); restored to the baseline serving state with a root-built config cache; no schema or data changed; `605a8dd` |
