...
final class HistoricalMakingCharge
{
    ...
    /**
     * The subset actually offered on the manual-entry dropdown (Batch 5
     * usability correction: "direct fixed amount, with percentage/per-gram
     * options when applicable"). `fixed_invoice`/`included`/`informational`/
     * `unknown` are disambiguation outcomes for an AMBIGUOUS printed column —
     * they only make sense on the file-import mapping screen, which keeps
     * using the full BASES list untouched. A manual operator already knows
     * their own arrangement, so this list is presentation-only: every basis
     * in BASES (including the ones left off this list) is still validated
     * and calculated exactly as before if it ever arrives in a request.
     */
    public const BASES_MANUAL_ENTRY = [
        self::BASIS_FIXED_LINE,
        self::BASIS_PERCENT,
        self::BASIS_PER_GRAM,
    ];
