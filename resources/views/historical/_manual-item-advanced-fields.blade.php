...
<div class="{{ $isDesktopGrid ? 'divide-y divide-slate-200 2xl:grid 2xl:grid-cols-12 2xl:gap-2 2xl:divide-y-0' : 'space-y-3' }}" data-historical-advanced-layout="{{ $itemGridMode }}">
    ...
    <section class="{{ $groupClass }} {{ $isDesktopGrid ? '2xl:col-span-5' : '' }}" data-historical-advanced-group="additional-charges">
        <div class="{{ $fieldsClass }}">
            ...
            @foreach(['hallmark' => 'Hallmark', 'rhodium' => 'Rhodium', 'other' => 'Other charge'] as $key => $label)
                @php $field = "line_{$key}_charge"; @endphp
                <div class="{{ $width('w-[120px]') }}"><label :for="`line-${i}-{{ $key }}-${@js($itemGridMode)}`">{{ $label }}</label><div class="relative"><input type="number" min="0" step="any" :id="`line-${i}-{{ $key }}-${@js($itemGridMode)}`" :name="`lines[${i}][{{ $field }}]`" x-model="line.{{ $field }}" :disabled="{{ $itemControlDisabled }}" class="{{ $fieldClass }} pr-8 text-right tabular-nums"><span class="{{ $unitClass }}">₹</span></div></div>
            @endforeach
        </div>
    </section>
