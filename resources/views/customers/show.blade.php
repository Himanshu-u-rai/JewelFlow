            ...
            @can('customers.delete')
                @if($customer->is_active)
                    <form action="{{ route('customers.archive', $customer) }}" method="POST" class="inline"
                        ...
                        @csrf
                        @method('PATCH')
                        <button type="submit" aria-label="Archive this customer" class="jf-header-action--warn inline-flex items-center justify-center gap-1.5 px-4 rounded-lg text-sm font-semibold focus:outline-none focus-visible:ring-2 focus-visible:ring-amber-500 focus-visible:ring-offset-2 transition-colors">Archive</button>
                    </form>
                @else
                    <form action="{{ route('customers.reactivate', $customer) }}" method="POST" class="inline">
                        @csrf
                        @method('PATCH')
                        <button type="submit" aria-label="Reactivate this customer" class="jf-header-action--success inline-flex items-center justify-center gap-1.5 px-4 rounded-lg text-sm font-semibold focus:outline-none focus-visible:ring-2 focus-visible:ring-emerald-500 focus-visible:ring-offset-2 transition-colors">Reactivate</button>
                    </form>
                @endif
            @endcan
