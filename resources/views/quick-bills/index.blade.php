<x-app-layout>
    ...
    <style>
        .qb-page {
            ...
            --qb-line:#e6e8ec; --qb-fill:#f6f7f9; --qb-ease:cubic-bezier(0.23,1,0.32,1);
        }
        ...
        /* ---- KPI cards ---- */
        .qb-stats { display: grid; grid-template-columns: repeat(5, 1fr); gap: 14px; margin-bottom: 18px; }
        .qb-stat {
        ...
        .qb-stat__icon { width: 34px; height: 34px; flex: 0 0 auto; border-radius: 9px; display: grid; place-items: center; }
        ...
        .qb-stat__value.is-money { font-size: 18px; }
        ...
        .qb-table thead th {
            ...
            color: var(--qb-muted); padding: 12px 20px; background: #fafbfc; border-bottom: 1px solid var(--qb-line);
        }
        .qb-table thead th.ar { text-align: right; }
        .qb-table tbody td { padding: 14px 20px; border-bottom: 1px solid #f1f3f7; vertical-align: middle; }
        ...
        .qb-action { display: inline-flex; align-items: center; gap: 6px; padding: 7px 12px; border-radius: 9px;
        ...
        .qb-pagination { padding: 14px 20px; border-top: 1px solid var(--qb-line); }
        .qb-empty { padding: 56px 24px; text-align: center; }

        /* Mobile card list (hidden on desktop) */
        .qb-cards { display: none; padding: 14px; }
        .qb-card { background: #fff; border: 1px solid var(--qb-line); border-radius: 13px; padding: 15px 16px; }
        .qb-card + .qb-card { margin-top: 12px; }
        .qb-card__top { display: flex; align-items: flex-start; justify-content: space-between; gap: 12px; }
        .qb-card__amounts { display: flex; gap: 24px; margin-top: 13px; padding-top: 12px; border-top: 1px solid #f1f3f7; }
        ...
        .qb-card__foot .qb-action { flex: 1; justify-content: center; padding-top: 9px; padding-bottom: 9px; }

        /* Entrance */
        @keyframes qb-rise { from { opacity: 0; transform: translateY(10px); } to { opacity: 1; transform: translateY(0); } }
        .qb-stats, .qb-panel { opacity: 0; animation: qb-rise .45s var(--qb-ease) forwards; }
        .qb-stats { animation-delay: .02s; }
        .qb-panel { animation-delay: .1s; }
        ...
        @media (max-width: 760px) {
            /* swap the inner table for the card list; keep the toolbar + panel */
            .qb-table, .qb-pagination { display: none; }
            .qb-cards { display: block; }
            .qb-toolbar { border-bottom: 0; }
            .qb-search { flex: 1 1 100%; }
            .qb-controls { margin-left: 0; width: 100%; }
            .qb-select { flex: 1 1 auto; }
            .qb-daterange { flex: 1 1 100%; justify-content: space-between; }
            .qb-daterange input { width: auto; flex: 1 1 0; }
        }
        @media (max-width: 560px) {
            .qb-stats { grid-template-columns: repeat(2, 1fr); gap: 12px; }
            .qb-stat { padding: 13px; gap: 9px; }
            .qb-stat__value { font-size: 19px; }
            .qb-stat__value.is-money { font-size: 15px; }
            .qb-stat:last-child:nth-child(odd) { grid-column: 1 / -1; }
        }

        @media (prefers-reduced-motion: reduce) {
            .qb-stats, .qb-panel { opacity: 1; animation: none; }
            .qb-stat, .qb-new-btn, .qb-action, .qb-clear, .qb-table tbody tr,
            .qb-search, .qb-select, .qb-daterange { transition: none; }
        }
    </style>
    ...
    <div class="content-inner max-w-[1380px] mx-auto qb-page">
        ...
        <div class="qb-stats">
            <div class="qb-stat">
                <span class="qb-stat__icon is-slate"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.9"><path stroke-linecap="round" stroke-linejoin="round" d="M9 7h6M9 11h6M9 15h4M6 3h12a1 1 0 011 1v17l-3-2-2 2-2-2-2 2-2-2-3 2V4a1 1 0 011-1z"/></svg></span>
                <div class="qb-stat__body"><div class="qb-stat__label">Total bills</div><div class="qb-stat__value">{{ number_format($stats['total_count']) }}</div></div>
            </div>
            <div class="qb-stat">
                <span class="qb-stat__icon is-green"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M20 6L9 17l-5-5"/></svg></span>
                <div class="qb-stat__body"><div class="qb-stat__label">Issued</div><div class="qb-stat__value">{{ number_format($stats['issued_count']) }}</div></div>
            </div>
            <div class="qb-stat">
                <span class="qb-stat__icon is-amber"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.9"><path stroke-linecap="round" stroke-linejoin="round" d="M11 4H4a2 2 0 00-2 2v14a2 2 0 002 2h14a2 2 0 002-2v-7M18.5 2.5a2.12 2.12 0 013 3L12 15l-4 1 1-4 9.5-9.5z"/></svg></span>
                <div class="qb-stat__body"><div class="qb-stat__label">Drafts</div><div class="qb-stat__value">{{ number_format($stats['draft_count']) }}</div></div>
            </div>
            <div class="qb-stat">
                <span class="qb-stat__icon is-gold"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.9"><path stroke-linecap="round" stroke-linejoin="round" d="M12 1v22M17 5H9.5a3.5 3.5 0 000 7h5a3.5 3.5 0 010 7H6"/></svg></span>
                <div class="qb-stat__body"><div class="qb-stat__label">Today's value</div><div class="qb-stat__value is-money">₹{{ number_format((float) $stats['today_total'], 2) }}</div></div>
            </div>
            <div class="qb-stat">
                <span class="qb-stat__icon is-amber"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.9"><path stroke-linecap="round" stroke-linejoin="round" d="M12 9v4M12 17h.01M10.3 3.9L1.8 18a2 2 0 001.7 3h17a2 2 0 001.7-3L13.7 3.9a2 2 0 00-3.4 0z"/></svg></span>
                <div class="qb-stat__body"><div class="qb-stat__label">Outstanding</div><div class="qb-stat__value is-money">₹{{ number_format((float) $stats['outstanding_total'], 2) }}</div></div>
            </div>
        </div>
        ...
        <div class="qb-panel">
            ...
            {{-- Toolbar. Filters apply on change (status/date) or on Enter/blur
                 (search), so there is no separate Filter button; Clear shows only
                 when a filter is active. --}}
            <form method="GET" action="{{ route('quick-bills.index') }}" class="qb-toolbar" data-qb-filter-form>
                <label class="qb-search">
                ...
                <div class="qb-controls">
                    <select name="status" class="qb-select" data-qb-autosubmit>
    ...
    <script>
        // Toolbar auto-submit: status + date apply on change; search on Enter
        // (native) or blur. No separate Filter button. Rebinds on Turbo load.
        (function () {
            const bind = () => {
